...
class App {
  ...
  constructor() {
    ...
    this.router = new Router([
      ...
      {
        url: "^/workoutExcercises/(.*)$",
        show: (matches) => -this._gotoWorkoutExcercises(matches[1]),
      },
      //last one
   ...
   * @param {Number} id ID der zu bearbeitenden Adresse
   */
       async _gotoWorkoutExcercises(id) {
        try {
          // Dynamischer Import, vgl. https://javascript.info/modules-dynamic-imports
          let { default: PageWorkout } = await import(
            "./page-workoutExcercises/page-workoutExcercises.js"
          );
      
          let page = new PageWorkout(this, id);
          await page.init();
          this._showPage(page, "workoutExcercises");
        } catch (ex) {
          this.showException(ex);
        }
      }
