...
export default class WorkoutOverviewhinzufuegen extends Page {
   ...
   */
   async saveExerciseinWorkout(exercise){     
    ...
    let answer = confirm(
      ...
      if (!answer) return;
    let test = await this._app.backend.fetch("GET", `/workout/${this._dataset._id}`);
    this._exercise;
  ...
  //     // Warum hier? Workout hinzufügen Jan
      ...
      liElement
          ...
          .addEventListener(
            ...
            "click", this.saveExerciseinWorkout(this.exercise)                           
